...
// Types
type Student = SemesterGradeReportData['students'][0]
type Subject = Student['subjects'][0]
type Summary = SemesterGradeReportData['summary']

export interface SemesterGradeReportData {
  academicYear: string
  semester: string
  year: string
  class?: string
  section?: string
  semesterMonths?: string[]
  lastSemesterMonth?: string
  previousSemesterMonths?: string[]
  students: {
    studentId: string
    firstName: string
    lastName: string
...
export function generateSemesterGradeReportHTML(data: SemesterGradeReportData): string {
  ...
  // Get semester months from actual data provided by API
  const semesterMonths = data.semesterMonths || []
  const finalMonth = semesterMonths[semesterMonths.length - 1]
  const previousMonths = semesterMonths.slice(0, -1)
  
  // Calculate semester-specific averages for each student
  const studentsWithSemesterData = data.students.map(student => {
    const semesterAverage = student.lastMonthAverage || student.averageGrade
    const monthlyAverage = student.previousMonthsAverage || 0
...
<head>
  ...
  <title>របាយការណ៍ពិន្ទុប្រចាំឆមាស</title>
  <style>
    ...
    * {
    ...
    th:nth-last-child(4), td:nth-last-child(4) { /* មធ្យមភាគខែប្រចាំឆមាស */
      width: 45px;
    }
    
    th:nth-last-child(5), td:nth-last-child(5) { /* មធ្យមភាគឆមាស */
      width: 45px;
    }
    
    th:nth-last-child(6), td:nth-last-child(6) { /* ពិន្ទុសរុបឆមាស */
      width: 52px;
    }
...
<body>
  <div class="document">
    ...
    <div class="report-title">
      បញ្ជីពិន្ទុប្រចាំឆមាសទី ${data.semester} ឆ្នាំសិក្សា ${data.academicYear} ថ្នាក់ទី ${data.class || 'N/A'}${data.section ? data.section : ''}
    </div>
    ...
    ` : !hasScores ? `
      <div class="no-data-message">
        <div class="no-data-title">⚠️ មិនមានពិន្ទុសម្រាប់ឆមាសនេះ</div>
        <div class="no-data-subtitle">ឆមាសទី ${data.semester} ឆ្នាំសិក្សា ${data.academicYear} មិនមានពិន្ទុបានបញ្ចូល</div>
      </div>
    ` : `
    <table>
      <thead>
        <tr>
          ...
          <th rowspan="2">ពិន្ទុសរុបឆមាស</th>
          <th rowspan="2">មធ្យមភាគឆមាស</th>
          <th rowspan="2">មធ្យមភាគខែប្រចាំឆមាស</th>
          <th rowspan="2">មធ្យមភាគប្រចាំឆមាស</th>
          <th rowspan="2">និទ្ទេសប្រចាំឆមាស</th>
          <th rowspan="2">ចំណាត់ថ្នាក់ប្រចាំឆមាស</th>
        </tr>
        <tr>
        </tr>
      </thead>
      <tbody>
        ...
        <tr class="summary-row">
          ...
          <td>${getFemaleSemesterAverage()}</td>
          <td>${getFemaleMonthlyAverage()}</td>
...
export async function generateSemesterGradeReportPDF(data: SemesterGradeReportData, options: ReportOptions = {
  format: 'A4',
  ...
  const html = generateSemesterGradeReportHTML(data)
