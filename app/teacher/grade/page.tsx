...
export default function GradePage() {
  ...
  const meanScoresBySubject = recentGrades.reduce((acc, grade) => {
    if (!acc[grade.subject]) {
      acc[grade.subject] = { total: 0, count: 0, scores: [] }
    }
    acc[grade.subject].total += grade.grade
    acc[grade.subject].count += 1
    acc[grade.subject].scores.push(grade.grade)
    return acc
  }, {} as Record<string, { total: number; count: number; scores: number[] }>)

  // Calculate overall statistics
  const overallMean = recentGrades.length > 0 
    ? (recentGrades.reduce((sum, grade) => sum + grade.grade, 0) / recentGrades.length).toFixed(1)
    : 0

  const overallMedian = recentGrades.length > 0 
    ? recentGrades.sort((a, b) => a.grade - b.grade)[Math.floor(recentGrades.length / 2)].grade
    : 0

  const overallMode = recentGrades.length > 0 
  ...
  const modeScore = Object.keys(overallMode).length > 0
    ? Object.entries(overallMode).reduce((a, b) => overallMode[Number(a[0])] > overallMode[Number(b[0])] ? a : b)[0]
    : 0
